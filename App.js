...
export default function App() {
  ...
  const loadNotes = useCallback(
    async (targetMode = mode, silent = false) => {
      try {
        ...
        const normalized = data.map((entry) => normalize(entry));
        const nextVotes = {};
        const nextReports = {};

        normalized.forEach((entry) => {
          if (entry.clientVote === "like" || entry.clientVote === "dislike") {
            nextVotes[entry.id] = entry.clientVote;
          }
          if (entry.clientReported) {
            nextReports[entry.id] = true;
          }
        });
    ...
    () =>
      notes.filter(
        (entry) =>
          ...
          const posW = (n.likes || 0);
          const total = neg + posW;
          let scale = 1.0;
          let opacity = 1.0;
          ...
          return { ...n, markerScale: scale, markerOpacity: opacity };
      }),
    [notes, mode]
  );
  ...
  const deleteOwnNote = useCallback(
    (note) => {
      if (!note?.id || !note.canDelete || note.isLive) return;
            ...
            onPress: () => {
              void (async () => {
                try {
                  ...
                  setSelectedNoteId((current) => (current === note.id ? "" : current));
                  setShowNoteDetails(false);
                  setApiOnline(true);
                  setError("");
                  showSuccess("Son supprime");
                } catch (requestError) {
                  setApiOnline(Boolean(requestError?.status));
