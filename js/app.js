...
function geoModeUI(){if(cGeoGps)cGeoGps.style.display="inline-block";if(cGeoPin)cGeoPin.style.display="inline-block";if(cGeoGps)cGeoGps.classList.toggle("active",geoMode==="gps");if(cGeoPin)cGeoPin.classList.toggle("active",geoMode==="pin");if(cUse)cUse.textContent=geoMode==="pin"?"Poser pin au centre":"Mettre a jour GPS";if(cHint)cHint.textContent=geoMode==="pin"?"Cliquez sur la carte pour placer votre pin (ou posez-le au centre).":"Votre publication prendra votre position GPS actuelle.";}
...
modeUI(false);geoModeUI();recUI();liveUI();compPos();zone(pos.lat,pos.lng);stats();void load(false);setInterval(()=>void load(true),POLL_MS);setTimeout(()=>toast("Cliquez sur une bulle pour ecouter","info"),700);
